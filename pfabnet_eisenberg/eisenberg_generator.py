...
from utils import generate_eisenberg_grids
...
if __name__ == '__main__':
    ...
    parser.add_argument('--eisenberg_output_dir', type=str, default='./',
                        help='directory to save the generated Eisenberg grid files')
    ...
    eisenberg_dir = in_args.eisenberg_output_dir
    seed = in_args.seed
    ...
    for mol_file in mol_files:
        print(mol_file)
        output = generate_eisenberg_grids(args, mol_file)
        for idx, (esp_grid, phobic_grid, philic_grid, output_mol) in enumerate(output):
            output_dir = os.path.join(eisenberg_dir, 'rotation_%d' % (idx + 1))
            try:
                os.mkdir(output_dir)
            except Exception as e:
                pass
            ...
            with open(esp_file, 'wb') as fptr:
               pickle.dump([esp_grid, phobic_grid, philic_grid], fptr)
