...
from utils import generate_eisenberg_grids, get_eisenberg_grids
from utils import DEFAULT_GRID_PARAMS, EISENBERG_DIR_KEY
...
def predict(cnn_models, mol_file, args = DEFAULT_GRID_PARAMS):
    ...
    if args['num_channels'] == 3:
        combined_grids = [np.concatenate([esp_arr, phobic_arr, philic_arr], axis=0)
                         for esp_arr, phobic_arr, philic_arr, _ in esp_grids]
    else:
        combined_grids = [np.concatenate([phobic_arr, philic_arr], axis=0)
                         for _, phobic_arr, philic_arr, _ in esp_grids]

    dummy_y = [0.0]*len(combined_grids)

    test_dataset = ViscosityDataset(combined_grids, dummy_y)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_channels', type=int, default=2,
                        help='number of input channels (2 for eisenberg '
                             'phobic + philic or 3 for esp_eisenberg (default 2))')
    parser.add_argument('--processors', type=int, default=5,
                        help='Number of CPUs for ESP grid calculation (default 5)')
    parser.add_argument('--eisenberg_dir', type=str, default='', help='directory with precomputed density grids')
